...
def batch_get_hidden_fur(model, tokenizer, prompts,k,n,pos,ins_hiddens,insert_positions,insert_le=1):
    batch_size = k
    answers = []
    tim = 0
    ptt = 0
    ...
    for batch_input in tqdm(batch_split(prompts, batch_size)):
        ...
        ins_hidd = ins_hiddens[v].unsqueeze(0)
        ins_hidd = ins_hidd.repeat(k,1,1,1)
        if ptt == 0:
            ptt = 1
            ins = encode_inputs["input_ids"]
            print(tokenizer.convert_ids_to_tokens(ins[0]))
        with torch.no_grad():
            outp = model(**encode_inputs, output_hidden_states = True, return_dict = True,out_attn=True,ins_attn=True,insert_pos=ins_p,insert_hiddens = ins_hidd,insert_len = ins_le,idea=1,ins_all=1,get_train=1,alpha = alpha)#["hidden_states"]
...
def train(model, tokenizer, prompts,k,n,pos,hiddens,insert_positions,insert_le=1,epoch=1):
    u_hidden = hiddens
    logits = 0
    for i in range(epoch):
        #print(u_hidden.shape)
        hiddens_,logits = batch_get_hidden_fur(model,tokenizer,prompts,k,n,pos,u_hidden,insert_positions,insert_le)
        u_hidden = hiddens_
    return u_hidden,logits
    ...
    for task in TASKS:
        ...
        for no_ in range(10):
            ...
            for i in range(test_df.shape[0]):
                ...
                train_prompt = gen_prompt(dev_df, task)
                prompt = train_prompt + prompt_end
                while len(tokenizer.tokenize(prompt)) + 1> 4096: # bos token
                    prompt_split = prompt.split("\n\n")
                    prompt_split.pop(1)
                    prompt = '\n\n'.join(prompt_split)
                label = str(test_df.iloc[i, test_df.shape[1]-1])
            ...
            for ly in range(0,tot_l+1):
                ...
                if ly == 0:
                    ...
                    ep = int(args.epoch)-1
                    hiddens,logits = train(model,tokenizer,[record['prompt'] for record in records_1shots],k,test_df.shape[0],pos=-2,hiddens=hiddens,insert_positions=-2,insert_le=2,epoch=ep)
                    run_results = {}
                    hiddens_ = hiddens
                else:
                    hiddens = hiddens_
